...
|---|---|---|---|---|---|
| File loading | All core tables | Verified that the raw CSV files could be loaded from the repository path | All required CSV files loaded successfully: `website_sessions`, `website_pageviews`, `orders`, `order_items`, `order_item_refunds`, and `products` | Confirms the repository structure and file access are working correctly for analysis | No action needed |
...
| Table structure | `website_pageviews` | Reviewed row and column count | Table contains 1,188,124 rows and 4 columns | Pageview volume is plausible relative to session volume and supports page-level funnel analysis | No action needed |
...
| Column validation | `website_pageviews` | Compared column names to the data dictionary | All expected fields are present, including pageview ID, timestamp, session ID, and page URL | Confirms the table supports landing-page, funnel, and page-level behavior analysis | No action needed |
...
| Data types | `website_pageviews` | Reviewed data types for keys, foreign keys, URL, and timestamp | `website_pageview_id` and `website_session_id` are integers; `pageview_url` is string; `created_at` is stored as string | Pageview keys and URLs are ready for analysis; time-based pageview trends require datetime conversion | Convert `created_at` to datetime during cleaning |
| Data types | `orders` | Reviewed data types for keys, measures, and timestamp | IDs and counts are integers; `price_usd` and `cogs_usd` are numeric; `created_at` is stored as string | Revenue and cost fields are ready for aggregation, but time-based analysis requires datetime conversion | Convert `created_at` to datetime during cleaning |
| Data types | `order_items` | Reviewed data types for keys, flags, measures, and timestamp | IDs and flag fields are integers; `price_usd` and `cogs_usd` are numeric; `created_at` is stored as string | Item-level revenue and cost are ready for aggregation, but time analysis requires datetime conversion | Convert `created_at` to datetime during cleaning |
| Data types | `order_item_refunds` | Reviewed data types for keys, measures, and timestamp | IDs are integers; `refund_amount_usd` is numeric; `created_at` is stored as string | Refund amount is ready for aggregation, but refund trend analysis requires datetime conversion | Convert `created_at` to datetime during cleaning |
...
| Missing values | `website_pageviews` | Checked missing values in keys, timestamp, session ID, and page URL | No missing values found | Page-level behavior data is structurally complete | No cleaning required for missing values |
| Missing values | `orders` | Checked missing values in keys, foreign keys, date fields, and money fields | No missing values found | Core order data is structurally complete | No cleaning required for missing values |
| Missing values | `order_items` | Checked missing values in keys, foreign keys, date fields, and money fields | No missing values found | Item-level transaction data is structurally complete | No cleaning required for missing values |
| Missing values | `order_item_refunds` | Checked missing values in keys, foreign keys, date fields, and money fields | No missing values found | Refund data is structurally complete | No cleaning required for missing values |
| Missing values | `products` | Checked missing values in keys, date fields, and product labels | No missing values found | Product dimension data is structurally complete | No cleaning required for missing values |

## Duplicate Row Checks

| Check Area | Table | What Was Tested | Finding | Impact | Planned Action / Status |
|---|---|---|---|---|---|
| Duplicate rows | All core tables | Checked for completely duplicated rows using `DataFrame.duplicated()` | Duplicate count is 0 for `website_sessions`, `website_pageviews`, `orders`, `order_items`, `order_item_refunds`, and `products` | No evidence of exact row-level duplication in the raw data | No action needed |

## Primary Key Checks

| Check Area | Table | What Was Tested | Finding | Impact | Planned Action / Status |
|---|---|---|---|---|---|
| Primary key uniqueness | All core tables | Reviewed key fields alongside missing-value and duplicate-row checks | `website_session_id`, `website_pageview_id`, `order_id`, `order_item_id`, `order_item_refund_id`, and `product_id` contain no null values and behave as unique record identifiers | Strong table-level integrity; records are reliable for joins, aggregations, and downstream KPI analysis | No action needed |

## Join Validation

| Check Area | Table | What Was Tested | Finding | Impact | Planned Action / Status |
|---|---|---|---|---|---|
| Join validation | `website_pageviews` vs `website_sessions` | Checked that every `website_pageviews.website_session_id` exists in `website_sessions.website_session_id` | 0 orphan rows found | Page-level activity can be safely connected to sessions for landing-page and funnel analysis | No action needed |
| Join validation | `orders` vs `website_sessions` | Checked that every `orders.website_session_id` exists in `website_sessions.website_session_id` | 0 orphan rows found | Joins from orders to sessions are reliable for conversion and attribution analysis | No action needed |
...
| Check Area | Table | What Was Tested | Finding | Impact | Planned Action / Status |
|---|---|---|---|---|---|
| Value plausibility | `orders` | Reviewed `items_purchased`, `price_usd`, and `cogs_usd` for nulls, negatives, zeros, and value ranges | `items_purchased` ranges from 1 to 2; `price_usd` ranges from 29.99 to 109.98; `cogs_usd` ranges from 9.49 to 41.98; no nulls, negatives, or zeros were found | Order-level quantities, prices, and costs appear plausible and suitable for KPI analysis | No cleaning required |
...
| Value plausibility | `order_item_refunds` | Reviewed `refund_amount_usd` for nulls, negatives, zeros, and value ranges | `refund_amount_usd` ranges from 29.99 to 59.99 with no nulls, negatives, or zeros | Refund values appear plausible and consistent with item-level pricing | No cleaning required |
...
Overall, the Fuzzy Factory data appears structurally healthy based on initial EDA. All six core tables have complete primary keys, zero fully duplicated rows, and validated relationships with zero orphan rows.

The addition of `website_pageviews` makes it possible to analyze more than acquisition and final conversion. The project can now examine landing pages, page-level traffic, key funnel steps, and how page behavior connects to session and order outcomes.

The main cleaning requirement is converting all `created_at` fields from string to proper datetime format before time-based filtering, grouping, trend analysis, and dashboard development.

## Open Cleaning Issues

- Convert all `created_at` fields from string to proper datetime types in each table.
- Decide how null UTM fields (`utm_source`, `utm_campaign`, and `utm_content`) and `http_referer` should be categorized in downstream marketing analysis, such as `untagged`, `direct`, `organic`, or `unknown`.
- Document all attribution assumptions in SQL models, KPI definitions, and Power BI dashboard notes.
- Create cleaned analytical views or tables that support session, pageview, order, product, and refund reporting.